...
class ArucoSingleTracker():
    ...
    def track(self, loop=True, verbose=False, show_video=None, frame=None):
        ...
        if not ids is None and self.id_to_find in ids[0]:
            ...
            x_avg = x_sum * .25
            y_avg = y_sum * .25
            self.x_ang = (x_avg - horizontal_res * .5) * (horizontal_fov / horizontal_res)
            self.y_ang = (y_avg - vertical_res * .5) * (vertical_fov / vertical_res)
            ...
            if verbose:
                print(
                    "Marker X = %.1f  Y = %.1f  Z = %.1f  - fps = %.0f" % (tvec[0], tvec[1], tvec[2], self.fps_detect))
