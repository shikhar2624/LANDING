            ...
            if ret:
                ...
                if marker_found:
                    ...
                    if self.land==1 or self.land2==1:
                        self.land2=1  
                        logger.info('landing to be started')
                        
                        ##land the drone
                        if self.vehicle.location.global_relative_frame.alt>=0.4:
                            logger.info("landing drone with control in z axis")
                            self.goto_position_target_local_ned(vx,vy,0.1)
                        else:
                            logger.info("landing drone with mode LAND activated")
